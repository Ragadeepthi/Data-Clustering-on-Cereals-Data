...
setwd("D://HoTes//CSE7305c_HoT_2383_Clustering_Batch29_2383")
...
#...change rownames from numbers to real names====
...
cereals_data[,cat_cols] <- data.frame(sapply(cereals_data[,cat_cols], as.factor))
str(cereals_data)
...
#...scaling the only numberic columns data==== 
cereals_scale_data <- data.frame(scale(cereals_data_imputed[,-c(2,3,10)], center = T, scale=T))
str(cereals_scale_data)

cereals_scale_data_m <- data.frame(scale(cereals_m_data[,-c(2,3,10)], center = T, scale=T))
cereals_scale_data_zero<- data.frame(scale(cereals_zero_imp_data[,-c(2,3,10)], center = T, scale=T))
cereals_scale_data_omit <- data.frame(scale(cereals_omit_data[,-c(2,3,10)], center =T, scale=T))
...
fviz_nbclust(cereals_scale_data, kmeans, method="silhouette", print.summary = F, k.max = 25)
...
#.....append cluster label to the actual data frame====
cereals_kmclust_data <-data.frame(cereals_scale_data, final_fit_kmeans$cluster)
head(cereals_kmclust_data)
...
Stabindex <- cluster_similarity(Actual_cluster, New_cluster, similarity = "jaccard", method="independence")
Stabindex
